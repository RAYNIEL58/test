...
function ReadSetting(retries)
    retries = (retries or 0) + 1
    if retries > 5 then
        return SaveToTable(TableToSave(Settings))
    end
...
local hitboxStartOld
pcall(function()
    local env = getrenv()
    if env and env._G and env._G.hitbox and type(env._G.hitbox.start) == "function" then
        hitboxStartOld = env._G.hitbox.start
        env._G.hitbox.start = function(...)
            local arg = {...}
            arg[3] = Vector3.new(30, 30, 30)
            return hitboxStartOld(unpack(arg))
        end
    end
end)
...
-- Removed setfflag calls to avoid NSE1 "cannot set invalid fflag" kick
-- if setfflag then
    ...
    function()
        ...
        not plr.PlayerGui:FindFirstChild("DEATHGUI") then 
            if CheckEN("Noclip") then 
                if (not setfflag or (identifyexecutor and (identifyexecutor() or ""):upper() == "KRNL")) then
                    if
...
function RayCast2(a, b, c)
    local ignored = {game:GetService("Workspace").Effects}
    if game.Players.LocalPlayer.Character then
        table.insert(ignored, game.Players.LocalPlayer.Character)
    end
...
function IsSea()
    ...
    if t.Instance and t.Instance.Name == "InvisSandd" then
        return true
    end
    return false
end
...
local current = tick()
local last = current
local function humadd(v)
    if v and v.Name == "Humanoid" and plr.Character and plr.Character:FindFirstChild("Humanoid") then
        pcall(function()
            v:GetPropertyChangedSignal("FloorMaterial"):Connect(function()
                if plr.Character and plr.Character.Humanoid and plr.Character.Humanoid.FloorMaterial == Enum.Material.Air then
                    last = tick()
                else
                    current = tick()
                end
